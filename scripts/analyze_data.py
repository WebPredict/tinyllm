...
def analyze_file(f):
    ...
    # --- SVG / path data detection ---
    svg_patterns = ["<svg", "<path", " d=\"M", " d=\"m", "viewBox="]
    svg_chars = sum(content.count(p) for p in svg_patterns)
    if svg_chars > 5 or (svg_chars > 0 and content.count("d=\"M") > 0 and len(content) > 500):
        issues.append("svg_path_data")

    # --- Translation / i18n files (just key-value pairs, no code logic) ---
    if content.count('": "') > 20 or content.count("': '") > 20:
        # Likely a JSON-style translation file embedded in TS
        issues.append("translation_data")

    # --- Long string literals / data dumps ---
    long_strings = sum(1 for line in lines if len(line) > 200 and ('"' in line or "'" in line))
    if long_strings > 10:
        issues.append("data_dump")

    # --- Auto-generated markers ---
    if any(marker in content[:500] for marker in [
        "auto-generated", "AUTO GENERATED", "DO NOT EDIT",
        "Generated by", "This file was generated", "codegen",
    ]):
        issues.append("auto_generated")
    ...
    # Negative signals
    if "svg_path_data" in issues: quality -= 25
    if "translation_data" in issues: quality -= 20
    if "data_dump" in issues: quality -= 15
    if "auto_generated" in issues: quality -= 20
